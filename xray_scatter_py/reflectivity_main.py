...
def main():
    data = Data1d(
        r'D:\OneDrive - University of Massachusetts\Umass Amherst'
    ...
    r_dist0 = calculator.refl_dist(
        data.qz, param_for_calc[0], param_for_calc[1:])
    # r_cont0 = calculator.refl_cont(data.qz, param_for_calc[0], param_for_calc[1:])

    # calculator.reset()

    popt_dist, pcov_dist = calculator.optimize(
        calculator.refl_dist, instr_broad, data, md1)
    ...
    reflectivity_pltlib.plt_refl_model(
        data.qz, data.rfl, 10**r_dist0, 'Experiment', 'r_dist0')
    # pltlib.plt_refl_model(data.qz, data.rfl, 10**r_cont0, 'Experiment', 'r_cont0')
    reflectivity_pltlib.plt_refl_model(
        data.qz, data.rfl, 10**r_dist, 'Experiment', 'Model')
