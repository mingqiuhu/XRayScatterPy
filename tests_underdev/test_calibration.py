...
class TestCalculateAngleAndQ(unittest.TestCase):
    ...
    def setUp(self):
        self.detx0 = 100.4
        self.params_dict_list, self.image_array = utils.read_multiimage(
            DATA_PATH, START_INDEX, END_INDEX)

    def test_calculate_angle(self):
        theta_array, azimuth_array = calibration.calculate_angle(
            self.detx0, self.params_dict_list, self.image_array)

        # Check if the returned arrays have the same shape as the input image
        # array
