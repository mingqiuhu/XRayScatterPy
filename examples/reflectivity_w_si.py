...
DATA_PATH = os.path.join(os.path.dirname(__file__), '..', 'data', 'tiff_files_w_si')
START_INDEX = 61397
END_INDEX = 61597
DETX0 = 100.4
QY_FWHM = 0.1
...
qz_1d, reflectivity_array, spillover_array, total_array = reflectivity.calculate_relative_reflectivity(QY_FWHM, 0.1, qy_array, qz_array, params_dict_list, image_array)
normalized_reflectivity, fitted_spillover = reflectivity.calculate_normalized_reflectivity(params_dict_list, qz_1d, reflectivity_array, spillover_array)

theta_1d = np.degrees(np.arcsin(qz_1d * 1.542 / 4 / np.pi))
data_plotting.plot_1d(qz_1d[:-5], normalized_reflectivity[:-5], yunit='normalized reflectivity', yticks=[1, 1e-2, 1e-4, 1e-6])
data_plotting.plot_1d_compare(theta_1d[:-5], spillover_array[:-5], theta_1d[:-5], fitted_spillover[:-5], xscale='linear', yscale='linear', xlabel='theta sample', ylabel='spill over',
