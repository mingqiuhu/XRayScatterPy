...
def calculate_synchrotron_abs(FILE_NAME, thickness, Q_MIN, Q_MAX, Q_NUM):
    ...
    i_1d = data_processing.calculate_1d(q_array, image_array_abs, sr_array,
                                        q_min=Q_MIN, q_max=Q_MAX, q_num=Q_NUM)
    np.savetxt(FILE_NAME + '.txt',
               np.stack((q_1d, i_1d[0, :] / factor), axis=-1))

    data_plotting.plot_2d(-qz_array,
                          qy_array,
                          image_array_abs / factor)
...
def calculate_synchrotron_abs_lowmemo(FILE_NAME, thickness, Q_MIN, Q_MAX, Q_NUM):
    DATA_PATH = os.path.join(
        os.path.dirname(__file__),
        '..',
        'data',
    ...
    q_1d = np.linspace(Q_MIN, Q_MAX, Q_NUM)
    i_1d = data_processing.calculate_1d_lowmemo(q_array, image_array_abs, sr_array,
                                                q_min=Q_MIN, q_max=Q_MAX, q_num=Q_NUM)
    np.savetxt(FILE_NAME + '.txt',
               np.stack((q_1d, i_1d[0, :] / factor), axis=-1))
