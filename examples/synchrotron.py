...
def calculate_synchrotron_abs(FILE_NAME, thickness, Q_MIN, Q_MAX, Q_NUM):
    ...
    i_1d = data_processing.calculate_1d(
        Q_MIN, Q_MAX, Q_NUM, q_array, image_array_abs, omega)
    np.savetxt(FILE_NAME + '.txt',
               np.stack((q_1d, i_1d[0, :] / factor), axis=-1))

    data_plotting.plot_2d_scattering(-qz_array,
                                     qy_array,
...
def calculate_synchrotron_abs_cheap(FILE_NAME, thickness, Q_MIN, Q_MAX, Q_NUM):
    ...
    q_1d = np.linspace(Q_MIN, Q_MAX, Q_NUM)
    i_1d = data_processing.calculate_1d_cheap(
        Q_MIN, Q_MAX, Q_NUM, q_array, image_array_abs, omega)
    np.savetxt(FILE_NAME + '.txt',
               np.stack((q_1d, i_1d[0, :] / factor), axis=-1))
...
calculate_synchrotron_abs_cheap(
    'ST_60s_exposure_2m', 0.1055, Q_MIN, Q_MAX, 300)
